...
WHERE starttime LIKE ('2012-09-14%') 
      AND ((b.memid <> 0 AND membercost >30)
	         OR (b.memid =0 AND guestcost *2 >30))
...
SELECT p.name facility_name, 
       concat(m.firstname, ' ', m.surname) AS member_name,
       cost
FROM   (SELECT memid, starttime, f.name,
               CASE WHEN b.memid <> 0 THEN membercost
                 ELSE guestcost *2 END cost
        FROM Bookings b
             LEFT JOIN Facilities f
                    ON b. facid = f.facid) p
        LEFT JOIN Members m
               ON p.memid = m.memid
WHERE starttime LIKE ('2012-09-14%')
      AND cost > 30
...
SELECT  m1.surname, 
        m1.firstname, 
        concat(m2.firstname, ' ', m2.surname) AS referred_by
FROM Members m1
